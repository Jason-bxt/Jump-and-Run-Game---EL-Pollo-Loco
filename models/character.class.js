class Character extends MovableObject {
  ...
  /**
   * Initiates the movement and animation cycles for the character.
   */
  animate() {
    setInterval(() => {
      this.handleMovement(); 
  ...
  /**
   * Handles idle animations based on key press timing.
   */
  handleIdleAnimation() {
    const currentTime = Date.now();
    if (!this.idleAnimationStarted && currentTime - this.lastKeyPressTime > 10 && !this.isLongIdle) {
  ...
  /**
   * Handles animations based on the character's state (dead, hurt, jumping, walking).
   */
  ...
  /**
   * Updates the last key press time.
   */
  updateKeyPressTime() {
